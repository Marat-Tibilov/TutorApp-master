...
class ScheduleScreen(Screen):
    ...
    def update_headers(self):  # обновление шапки с датой 
        self.ids.header_month.text = self.current_date.strftime("%B, %Y")
        self.ids.header_day.text = self.current_date.strftime("%d, %A")

    def update_content(self):               # загрузка всех уроков на определенный день 
    ...
    def add_lesson(self): # вызов окна с добавление уроков 
    ...
    def save(self, instance):  # ссылка на объект , вызывается в метод save когда добавляется изменение 
        ...
        data = self.dialog.content_cls.student   # запрос на добавление в базу 
        with self.app.con().cursor() as cursor:
